...
**[NixOS Modules](https://wiki.nixos.org/wiki/NixOS_modules)** ([`modules/nixos/`](modules/nixos/)) -- Standard NixOS modules following the `options`/`config` pattern (see [`example.nix`](modules/nixos/example.nix)). Each module is automatically wrapped so it (a) applies the universal overlay (giving you `pkgs.internal.*`, `pkgs.lib.internal.*`, `pkgs.inputs`) and (b) receives all flake context (`inputs`, `modules`, `overlays`, `packages`, etc.) as module arguments. This wrapping applies whether the module is consumed inside this flake's own `systems/` or imported by a downstream consumer through the exported `nixosModules` output -- the module behaves identically in both cases. Context args are injected with `mkDefault` priority, so a downstream consumer that already passes its own e.g. `inputs` via `specialArgs` wins. Import them in your system configurations via the `modules.nixos` tree.
