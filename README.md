...
**[NixOS Modules](https://wiki.nixos.org/wiki/NixOS_modules)** ([`modules/nixos/`](modules/nixos/)) -- Standard NixOS modules following the `options`/`config` pattern (see [`example.nix`](modules/nixos/example.nix)). Each module is automatically wrapped so it (a) applies the universal overlay (giving you `pkgs.internal.*`, `pkgs.lib.internal.*`, `pkgs.inputs`) and (b) receives all flake context (`inputs`, `modules`, `overlays`, `packages`, etc.) as module arguments. This wrapping applies whether the module is consumed inside this flake's own `systems/` or imported by a downstream consumer through the exported `nixosModules` output -- the module behaves identically in both cases. A downstream consumer's `specialArgs` still takes precedence over the injected context, so you can override `inputs`, `modules`, etc. from the outside. Import modules in your system configurations via the `modules.nixos` tree.
...
`scanDir` walks `dir` recursively and returns a tree mirroring its layout, with file-name keys stripped of the suffix. `./secrets/postgres/foo.age` becomes `extra.secrets.postgres.foo`. `./secrets/postgres/default.age` is addressable as `extra.secrets.postgres.default` -- siblings are preserved. The suffix must start with `.` (e.g. `".age"`, not `"age"`).
...
Inside `extraContext`, safe context fields to read are `src`, `inputs`, `lib`, `packages` (paths only), `templates`, and `overlays`. Reading `ctx.extra`, `ctx.systems`, or evaluating anything in `ctx.modules.nixos` would close a recursion loop back through `extraContext`.

The `lib` you receive is the scaffold's *internal* lib (what gets exposed under `lib.${namespace}` in modules), not nixpkgs's lib. Reach for `inputs.nixpkgs.lib` (or `inputs.scaffold.lib` for the helpers above) when you need them.
